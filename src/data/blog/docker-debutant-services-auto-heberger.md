...
Un serveur de streaming pour tes films, séries, musique. Open source, sans télémétrie, sans pub. Mon [guide Jellyfin](https://brandonvisca.com/jellyfin-docker-alternative-netflix-gratuite/) couvre la configuration GPU et le transcodage. Pour gérer les demandes de tes utilisateurs sans passer par des messages, j'ai aussi publié un guide sur [Jellyseerr avec Docker](/jellyseerr-docker-gestion-demandes/). Pour automatiser les téléchargements de films et séries, les guides [Radarr](/radarr-docker-gestion-films/) et [Sonarr](/sonarr-docker-gestion-series/) complètent parfaitement la stack. Pour la musique uniquement, j'ai aussi publié un guide sur [Navidrome avec Docker](/navidrome-docker-serveur-musique/), un serveur audio ultra-léger et dédié. Et si tu veux streamer tes jeux PC vers la TV du salon, j'ai un guide complet sur [Sunshine Docker pour le streaming de jeux](/sunshine-docker-streaming-jeux/).
...
Si Kubernetes te titille mais que tu trouves ça trop lourd, il existe une solution intermédiaire : [K3s Docker](/k3s-docker-kubernetes-leger-homelab/), une distribution Kubernetes allégée qui tourne sur un Raspberry Pi 4 et embarque Traefik pour le HTTPS.
