...
Mon avis : pour un serveur auto-hébergé avec Docker, **UFW reste le meilleur compromis**. À condition de corriger le problème iptables. iptables brut est surpuissant mais inutilement complexe quand on veut juste bloquer quelques ports. nftables est l'avenir, mais la plupart des tutos et scripts tiers (y compris Docker lui-même) parlent encore iptables. Si tu veux passer à nftables avec Docker et reprendre le contrôle total de ton pare-feu, j'ai un guide complet sur [nftables Docker : pare-feu nouvelle génération sous Linux](/nftables-docker-pare-feu-linux/). Si tu n'utilises pas Docker du tout, j'ai un guide de [hardening en 10 commandes natives](/hardening-linux-10-commandes/) qui couvre SSH, sysctl et Fail2Ban sans conteneur.
