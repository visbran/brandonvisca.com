...
Si tu cherches un accès distant sécurisé pour ton homelab sans vendre ton âme à un SaaS, Firezone Docker est aujourd'hui le meilleur compromis. Et si ton FAI te met en CGNAT sans IP publique du tout, [Cloudflare Tunnel Docker](/cloudflare-tunnel-docker-homelab/) te permet d'exposer tes services sur Internet sans ouvrir aucun port. Déploie-le, configure ton OIDC, et dors enfin tranquille en sachant que tes services ne sont accessibles qu'aux bonnes personnes, aux bons moments, et depuis les bons appareils.
