---
title: "Tianji : pourquoi j'ai lâché Google Analytics pour du self-hosted"
description: "Tianji : l'alternative open-source à Google Analytics en self-hosted. Analytics + uptime + server status, tout-en-un. Privacy-friendly, sans cookie banner."
...
- [Pourquoi je me suis dit "stop" avec Google Analytics](#pourquoi-je-me-suis-dit-stop-avec-google-analytics)
- [Tianji, c'est quoi exactement](#tianji-cest-quoi-exactement)
- [Les 4 piliers de Tianji](#les-4-piliers-de-tianji)
- [Pourquoi je préfère Tianji à Google Analytics](#pourquoi-je-préfère-tianji-à-google-analytics)
- [La mise en place (spoiler : c'est rapide)](#la-mise-en-place-spoiler--cest-rapide)
- [Les limites (parce qu'il faut être honnête)](#les-limites-parce-quil-faut-être-honnête)
- [FAQ](#faq)
- [Verdict](#verdict)
...
J'ai cherché une alternative. J'ai testé Umami, Plausible, Matomo. C'est bien. Mais j'ai fini par tomber sur **Tianji** — et là, ça a cliqué.

---

## Tianji, c'est quoi exactement

Tianji (GitHub : [msgbyte/tianji](https://github.com/msgbyte/tianji)) se définit comme un **"All-in-One Insight Hub"**. Traduction : c'est pas juste un outil d'analytics. C'est **analytics + uptime monitoring + server status + surveys + telemetry**, le tout dans une seule app.
...
Tianji est **self-hosted**. Tes données restent sur ton serveur. Pas de Google qui les récupère pour alimenter ses profils publicitaires. Pas de cookie tiers. **Pas de bannière de consentement** sur ton site — parce que tu ne partages rien avec personne. Le RGPD devient un non-sujet.

### All-in-One

Avant Tianji, mon stack monitoring ressemblait à ça :
- Umami pour les stats
- Uptime Kuma pour la dispo
...
Tianji s'installe avec Docker Compose. Un `docker-compose.yml` minimaliste, deux services : l'app et une base PostgreSQL.
...
Si tu en as marre du bloat de Google Analytics, des bannières de cookies, et du tracking invasif, **Tianji est une option solide**. C'est pas parfait, c'est pas aussi puissant que GA4 — mais c'est **100x plus agréable à utiliser** au quotidien.
